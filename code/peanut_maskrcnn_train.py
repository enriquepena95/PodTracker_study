...
dataset_dicts = DatasetCatalog.get("my_pod_dataset_train")
...
def prepare_cfg(num_epochs=20, debug=True):
    ...
    config_file = os.path.join(detectron2_folder, "configs/COCO-InstanceSegmentation/mask_rcnn_R_101_FPN_3x.yaml")
    ...
    #"/home/wolftech/jcdunne.lab/Downloads/model_final.pth"

    # Use GPU
    cfg.MODEL.DEVICE = "cuda"
    # Number of classes. Some tutorials add the background as second class - this is incorrect
    cfg.MODEL.ROI_HEADS.NUM_CLASSES = 1
    # Set the confidence threshold for this model (you can tune this during inference to get more or less detections)
    ...
    cfg.OUTPUT_DIR = os.path.join(models_folder, "pod_models")
...
# ## Inference - Skipping visualization part
# No longer attempting to display any images to avoid Qt errors
...
# Plot losses without displaying
try:
    ...
    if experiment_metrics:
        ...
        plt.savefig(os.path.join(plots_folder, 'maskrcnn_training_loss.png'), dpi=300)
        plt.close()  # Close the figure to prevent display
        print(f"Loss plot saved to {os.path.join(plots_folder, 'maskrcnn_training_loss.png')}")
    else:
...
# ## Get Average Precision Scores - This is what you care about
